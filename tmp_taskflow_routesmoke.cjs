...
(async()=>{
  ...
  const routes = ['#/','#/my-log','#/my-tasks','#/all-tasks','#/attendance','#/team-logs','#/review-export','#/import-review','#/settings'];
  for (const route of routes) {
    await page.goto('https://ennffie.github.io/taskflow-v2/' + route, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(5000);
    await logState(route);
  }
  await browser.close();
})();
